        ...
        <form action="{{ route('boats.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div>
                    ...
                    <input type="text" name="nombre" id="nombre" class="w-full px-4 py-3 text-sm text-gray-900 border border-gray-300 rounded-lg placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-gray-900 focus:border-transparent transition-all duration-200" value="{{ old('nombre') }}" required>
                    @error('nombre')
                ...
                <div>
                    ...
                    <input type="number" name="capacidad_pasajeros" id="capacidad_pasajeros" class="w-full px-4 py-3 text-sm text-gray-900 border border-gray-300 rounded-lg placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-gray-900 focus:border-transparent transition-all duration-200" value="{{ old('capacidad_pasajeros') }}" min="1" required>
                ...
                <div class="flex items-center gap-3">
                    <input type="checkbox" name="activo" id="activo" class="w-4 h-4 text-gray-900 border-gray-300 rounded focus:ring-gray-900" {{ old('activo', true) ? 'checked' : '' }}>
            ...
            <div>
                ...
                <textarea name="descripcion" id="descripcion" rows="4" class="w-full px-4 py-3 text-sm text-gray-900 border border-gray-300 rounded-lg placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-gray-900 focus:border-transparent transition-all duration-200 resize-none">{{ old('descripcion') }}</textarea>
                @error('descripcion')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex justify-end gap-4 pt-6 border-t border-gray-100">
                ...
                <button type="submit" class="px-6 py-2.5 text-sm font-medium text-white bg-gray-900 rounded-lg hover:bg-gray-800 tracking-wide transition-all duration-200 shadow-sm hover:shadow-md">Guardar Barco</button>
            </div>
        </form>
    </div>
