...
@section('header')
    <div class="flex items-center gap-4 animate-enter">
        ...
        <div>
            <h1 class="font-display text-2xl font-medium text-gray-900">Nueva Salida</h1>
        ...
        <form action="{{ route('departures.store') }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="barco_id" id="barco_id" class="w-full px-4 py-3 text-sm text-gray-900 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-900 focus:border-transparent transition-all duration-200" required>
                        ...
                        @foreach(\App\Models\Boat::where('activo', true)->orderBy('nombre')->get() as $boat)
                            <option value="{{ $boat->id }}" {{ old('barco_id') == $boat->id ? 'selected' : '' }}>
                ...
                <div class="space-y-2">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <input type="date" name="fecha_date" id="fecha_date" class="w-full px-3 py-2.5 text-sm text-gray-900 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-900 focus:border-transparent transition-all duration-200" value="{{ old('fecha_date', now()->format('Y-m-d')) }}">
                        </div>
                        <div>
                            <label for="fecha_time" class="block text-xs text-gray-500 mb-1">Hora</label>
                            <input type="time" name="fecha_time" id="fecha_time" class="w-full px-3 py-2.5 text-sm text-gray-900 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-900 focus:border-transparent transition-all duration-200" value="{{ old('fecha_time', '09:00') }}">
                        </div>
                    </div>
                ...
                <div>
                    ...
                    <input type="text" name="puerto_salida" id="puerto_salida" class="w-full px-4 py-3 text-sm text-gray-900 border border-gray-300 rounded-lg placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-gray-900 focus:border-transparent transition-all duration-200" value="{{ old('puerto_salida') }}" placeholder="ej: Puerto Ayora" required>
                    @error('puerto_salida')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="precio" id="precio" class="w-full pl-8 pr-4 py-3 text-sm text-gray-900 border border-gray-300 rounded-lg placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-gray-900 focus:border-transparent transition-all duration-200" value="{{ old('precio') }}" step="0.01" min="0" placeholder="0.00" required>
                    </div>
                    @error('precio')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="number" name="pasajeros_reservados" id="pasajeros_reservados" class="w-full px-4 py-3 text-sm text-gray-900 border border-gray-300 rounded-lg placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-gray-900 focus:border-transparent transition-all duration-200" value="{{ old('pasajeros_reservados', 0) }}" min="0" placeholder="0">
            ...
            <div class="flex justify-end gap-4 pt-6 border-t border-gray-100">
                ...
                <button type="submit" class="px-6 py-2.5 text-sm font-medium text-white bg-gray-900 rounded-lg hover:bg-gray-800 tracking-wide transition-all duration-200 shadow-sm hover:shadow-md">Crear Salida</button>
            </div>
        </form>
    </div>
