...
@section('header')
    <div class="flex justify-between items-center">
        <h1 class="text-2xl font-bold text-gray-900">Nueva Salida</h1>
...
@section('content')
    ...
    <div class="card">
        <form action="{{ route('admin.departures.store') }}" method="POST" class="space-y-6">
            @csrf
            ...
            <div>
                ...
                <select id="itinerary_type" name="itinerary_type" class="form-input" required>
                    ...
                    <option value="4D/3N" {{ old('itinerary_type') == '4D/3N' ? 'selected' : '' }}>4 Días / 3 Noches</option>
                    <option value="5D/4N" {{ old('itinerary_type') == '5D/4N' ? 'selected' : '' }}>5 Días / 4 Noches</option>
                    <option value="8D/7N" {{ old('itinerary_type') == '8D/7N' ? 'selected' : '' }}>8 Días / 7 Noches</option>
                </select>
            </div>
            ...
            <div>
                ...
                <input type="number"
                       ...
                       class="form-input"
                       value="{{ old('reserved_passengers', 0) }}"
                       min="0"
                       placeholder="0">
            </div>
